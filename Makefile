...
.PHONY: check-env venv install install-r build-kenlm build-wiki-arpa download-public-data notebook public-notebook run-public-replication onestop-extension-notebook run-onestop-extension mega-acceptability-extension-notebook run-mega-acceptability-extension emtec-extension-notebook run-emtec-extension
...
run-emtec-extension: emtec-extension-notebook
	cd src && $(COMMON_ENV) ../$(JUPYTER) nbconvert \
	  --to notebook \
	  --execute additional_plan_emtec_extension.ipynb \
	  --output additional_plan_emtec_extension.executed.ipynb \
	  --ExecutePreprocessor.timeout=-1 \
	  --ExecutePreprocessor.kernel_name=python3
