...
\begin{abstract}
The uniform information density (UID) hypothesis predicts that language users prefer signals whose information is distributed smoothly rather than concentrated in sharp surprisal spikes. We replicate the public-data portion of Meister et al.'s analysis of whether reading times and acceptability judgments are better predicted by linear or power-transformed surprisal. We also contribute three extensions: OneStop ordinary eye-tracking, MegaAcceptability clause-embedding judgments, and EMTeC eye movements on LLM-generated text. The public replication recovers the original qualitative contrast: acceptability judgments show clearer benefits from non-linear surprisal predictors than reading-time data. Scoring every extension with GPT-2, BERT, and a WikiText-103 5-gram model sharpens this contrast but also exposes model dependence: MegaAcceptability gives the clearest acceptability-side generalization, EMTeC gives a small reading-time signal, and OneStop remains weak at paragraph level.
\end{abstract}

\section{Introduction and literature review}

Information theory gives a compact way to connect linguistic prediction with human processing. The surprisal of a word $u_n$ is $s(u_n)=-\log p(u_n \mid u_{<n})$, so lower-probability words carry more information \cite{shannon1948,hale2001,levy2008}. Prior comprehension work shows that contextual surprisal predicts processing difficulty, often approximately linearly \cite{smithLevy2013,goodkind2018}. UID adds a related but stronger efficiency claim from the production literature: speakers and comprehenders should prefer utterances that avoid unnecessary peaks and troughs in information density \cite{levyJaeger2007}. Our goal is to test how far this efficiency claim extends beyond production, especially for comprehension and acceptability judgments.
...
Our project asks whether this prior result is reproducible and whether its boundary conditions are theoretically meaningful. Rather than treating reproducibility as exact numerical identity, we focus on a public, inspectable subset: CoLA \cite{warstadt2019} and BNC acceptability judgments \cite{lau2017}, Natural Stories self-paced reading \cite{futrell2018}, Provo eye movements \cite{luke2018}, GPT-2 \cite{radford2019}, BERT \cite{devlin2019}, and a WikiText-103/KenLM 5-gram model \cite{merity2017,heafield2011}. We then ask whether the same UID logic transfers to ordinary reading, structured acceptability, and reading of generated text using OneStop \cite{berzak2025}, MegaAcceptability \cite{whiteRawlins2020}, and EMTeC \cite{bolliger2025}.
...
The results are interesting because they separate two claims that are easy to conflate. Surprisal is useful for predicting human processing, but the distributional shape of surprisal is much more visible in acceptability judgments than in sentence-level reading times. After applying the same GPT-2, BERT, and KenLM scoring families to every extension, this contrast is not driven by one model or one dataset: MegaAcceptability strengthens the acceptability-side conclusion, OneStop remains near zero at paragraph level, and EMTeC connects UID-style predictors to LLM decoding decisions with small reading-time gains.

This connects directly to class material because the project starts from entropy, surprisal, and probabilistic language models, then asks whether an information-theoretic efficiency principle predicts cognition. The answer is graded rather than binary. UID is not simply confirmed or rejected; instead, it is strongest when the behavioral measure is acceptability, weaker for online reading times, and sensitive to the language model used to estimate surprisal.

The project opens several future directions. Empirically, Brown, Dundee, GECO, and TransfoXL should be restored where access and compatibility allow; the extensions should be harmonized at word, sentence, and trial levels; and eye-tracking analyses should move toward word-level mixed models where possible. For generated text, the next step is to compare surprisal from the actual text-generating models, not only GPT-2, BERT, and a 5-gram proxy. Conceptually, the extensions suggest using UID as a diagnostic for both human language processing and LLM decoding policies: different generation strategies create different information-density profiles for readers.
...
We thank the course staff for guidance and Meister et al. for releasing the original UID analysis code. We especially thank our professor for feedback after our presentation; in response, we revised the extension analyses so that each extension uses the same GPT-2, BERT, and WikiText-103 5-gram scoring families used in the main replication, which made the extension results more complete and easier to compare. We also acknowledge the creators of the public datasets and tools cited in this report.
